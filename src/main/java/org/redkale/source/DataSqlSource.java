...
public interface DataSqlSource extends DataSource {
    ...
    default <K, V> Map<K, V> nativeQueryMap(Class<K> keyType, Class<V> valType, String sql) {
        return nativeQuery(sql, rset -> {
            ...
            while (rset.next()) {
                if (!rset.wasNull()) {
                    map.put((K) formatColumnValue(keyType, rset.getObject(1)), (V)
                            formatColumnValue(valType, rset.getObject(2)));
                }
            }
            return map;
        });
    ...
    default <V> CompletableFuture<V> nativeQueryAsync(
            String sql, Function<DataResultSet, V> handler, Map<String, Object> params) {
    ...
    @ClassDepends
    default <K, V> Map<K, V> nativeQueryMap(
            Class<K> keyType, Class<V> valType, String sql, Map<String, Object> params) {
        return nativeQuery(
                sql,
                rset -> {
                    Map<K, V> map = new LinkedHashMap<>();
    ...
    @ClassDepends
    default <K, V> CompletableFuture<Map<K, V>> nativeQueryMapAsync(
            Class<K> keyType, Class<V> valType, String sql, Map<String, Object> params) {
        return nativeQueryAsync(
                sql,
                rset -> {
    ...
    default CompletableFuture<Map<Integer, String>> nativeQueryToIntStrMapAsync(
    ...
    default <V> V nativeQuery(String sql, Function<DataResultSet, V> handler, Serializable bean) {
        return nativeQuery(
                sql, null, handler, (Map<String, Object>) Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }

    default <V> CompletableFuture<V> nativeQueryAsync(
            String sql, Function<DataResultSet, V> handler, Serializable bean) {
        return nativeQueryAsync(
                sql, null, handler, (Map<String, Object>) Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }
    ...
    default <V> CompletableFuture<V> nativeQueryOneAsync(Class<V> type, String sql, Serializable bean) {
        return nativeQueryOneAsync(
                type, sql, (Map<String, Object>) Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }
    ...
    default <V> CompletableFuture<List<V>> nativeQueryListAsync(Class<V> type, String sql, Serializable bean) {
        return nativeQueryListAsync(
                type, sql, (Map<String, Object>) Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }

    default <K, V> Map<K, V> nativeQueryMap(Class<K> keyType, Class<V> valType, String sql, Serializable bean) {
        return nativeQueryMap(
                keyType, valType, sql, (Map<String, Object>) Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }

    default <K, V> CompletableFuture<Map<K, V>> nativeQueryMapAsync(
            Class<K> keyType, Class<V> valType, String sql, Serializable bean) {
        return nativeQueryMapAsync(
                keyType, valType, sql, (Map<String, Object>) Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }

    default Map<String, String> nativeQueryToStrStrMap(String sql, Serializable bean) {
        return nativeQueryMap(String.class, String.class, sql, (Map<String, Object>)
    ...
    default CompletableFuture<Map<String, String>> nativeQueryToStrStrMapAsync(String sql, Serializable bean) {
        return nativeQueryMapAsync(String.class, String.class, sql, (Map<String, Object>)
                Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }

    default Map<Integer, String> nativeQueryToIntStrMap(String sql, Serializable bean) {
        return nativeQueryMap(Integer.class, String.class, sql, (Map<String, Object>)
                Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }

    default CompletableFuture<Map<Integer, String>> nativeQueryToIntStrMapAsync(String sql, Serializable bean) {
        return nativeQueryMapAsync(Integer.class, String.class, sql, (Map<String, Object>)
                Copier.copyToMap(bean, Copier.OPTION_SKIP_NULL_VALUE));
    }
    ...
    default <V> Sheet<V> nativeQuerySheet(Class<V> type, String sql, PageBean pageBean) {
        return nativeQuerySheet(
                ...
                pageBean == null
                        ? null
                        : (Map<String, Object>) Copier.copyToMap(pageBean.getBean(), Copier.OPTION_SKIP_NULL_VALUE));
    }
