...
public abstract class JsonDynEncoder<T> implements Encodeable<JsonWriter, T> {
    ...
    protected static JsonDynEncoder generateDyncEncoder(final JsonFactory factory, final Class clazz, final List<AccessibleObject> members) {
        ...
        try {
            ...
            JsonDynEncoder resultEncoder = (JsonDynEncoder) newClazz.getDeclaredConstructor(JsonFactory.class, Type.class).newInstance(factory, clazz);
        ...
        cw.visit(V11, ACC_PUBLIC + ACC_FINAL + ACC_SUPER, newDynName, "L" + supDynName + "<" + valtypeDesc + ">;", supDynName, null);
...

        {
            ...
            if (onlyOneLatin1FieldObjectFlag) {
                ...
                mv.visitMethodInsn(INVOKEVIRTUAL, writerName, "writeObjectByOnlyOneLatin1FieldValue", "([BLjava/lang/String;)V", false);
                maxLocals++;
                ...
                if (element1 instanceof Field) {
                    mv.visitFieldInsn(GETFIELD, valtypeName, ((Field) element1).getName(), org.redkale.asm.Type.getDescriptor(fieldtype1));
                } else {
                ...
                if (element2 instanceof Field) {
                    mv.visitFieldInsn(GETFIELD, valtypeName, ((Field) element2).getName(), org.redkale.asm.Type.getDescriptor(fieldtype2));
                } else {
                ...
                for (AccessibleObject element : members) {
                    ...
                    mv.visitVarInsn(ALOAD, 1);
                    mv.visitVarInsn(ALOAD, 0);
                    mv.visitFieldInsn(GETFIELD, newDynName, fieldname + (elementIndex == 0 ? "FirstFieldBytes" : "CommaFieldBytes"), "[B");
                    ...
                    if (fieldtype == short.class) {
                        mv.visitMethodInsn(INVOKEVIRTUAL, writerName, elementIndex + 1 == membersSize ? "writeLastFieldShortValue" : "writeFieldShortValue", "([BS)V", false);
                    } else if (fieldtype == int.class) {
                        mv.visitMethodInsn(INVOKEVIRTUAL, writerName, elementIndex + 1 == membersSize ? "writeLastFieldIntValue" : "writeFieldIntValue", "([BI)V", false);
                    } else if (fieldtype == long.class) {
                        mv.visitMethodInsn(INVOKEVIRTUAL, writerName, elementIndex + 1 == membersSize ? "writeLastFieldLongValue" : "writeFieldLongValue", "([BJ)V", false);
                    } else {
                        mv.visitMethodInsn(INVOKEVIRTUAL, writerName, elementIndex + 1 == membersSize ? "writeLastFieldLatin1Value" : "writeFieldLatin1Value", "([BLjava/lang/String;)V", false);
                    }
        ...
        RedkaleClassLoader.putReflectionDeclaredConstructors(newClazz, newDynName.replace('/', '.'), JsonFactory.class, Type.class);
        try {
            JsonDynEncoder resultEncoder = (JsonDynEncoder) newClazz.getDeclaredConstructor(JsonFactory.class, Type.class).newInstance(factory, clazz);
            Field selfField = newClazz.getDeclaredField("objectEncoderSelf");
            selfField.setAccessible(true);
